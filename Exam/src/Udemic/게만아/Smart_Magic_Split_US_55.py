...
for stock_data in TargetStockList:
    ...
    df = Common.GetOhlcv("US",stock_code, 200) ### 일봉정보를 가져온다 200개!
    #####################################
            ...
            if MagicDataInfo['StockCode'] == stock_code:
                ...
                if IsFullBuy == True:
                    ...
                    if LastRate <= LastSplitMeta['trigger_rate']:
                        ...
                        if IsOver == True:
                            msg = stock_code + " 스마트스플릿 풀매수 상태여인데 1차수 매도할 수량이 보유 수량보다 많은 상태라 모두 매도함!" + str(SellAmt) + "주 매도!"
