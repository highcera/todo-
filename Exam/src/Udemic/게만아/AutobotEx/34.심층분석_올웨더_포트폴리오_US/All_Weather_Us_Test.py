...
올웨더 포트폴리오의 반전!
https://blog.naver.com/zacra/223153229412
...
#BackTestType = 'A' #자산가형 
BackTestType = 'B' #초심자 공격형

################################################################

InvestStockList = ["TLT","SPY","IEF","GLD","DBC","IYK"]
...
for stock_code in InvestStockList:
    ...
    if stock_code == "TLT":
        stock_data['target_rate'] = 0.27
    elif stock_code == "SPY":
        stock_data['target_rate'] = 0.2
    elif stock_code == "IEF":
        stock_data['target_rate'] = 0.15
    elif stock_code == "GLD":
        stock_data['target_rate'] = 0.08
    elif stock_code == "DBC":
        stock_data['target_rate'] = 0.08
    elif stock_code == "IYK":
        stock_data['target_rate'] = 0.22
...
for stock_code in InvestStockList:
    ...
    #################################################################
    #################################################################
    df = Common.GetOhlcv("US", stock_code,6100) 
    #################################################################
    #################################################################
...
for date in combined_df.index.unique():
    ...
    #SPY가 150,50 이평선 밑이라면 무조건 지옥이다!!
    if SPY_data['150ma'].values[0] > SPY_data['prevClose'].values[0] and SPY_data['50ma'].values[0] > SPY_data['prevClose'].values[0]  :#and IYK_data['200ma'].values[0] > IYK_data['prevClose'].values[0]:
        IsHell = True
            ...
            if len(stock_data) == 1:
                ...
                if BackTestType == 'A':
                    ...
                    if IsHell == True:
                        ...
                        if investData['stock_code'] in ['SPY',"IYK"]:
                            Rate = 0.25
                        elif investData['stock_code'] in ['TLT',"IEF"]:
                            Rate = 1.75
                ...
                #B: 초심자 공격형
                    ...
                    if stock_data['120ma'].values[0] > stock_data['prevClose'].values[0]:
                        if investData['stock_code'] in ['GLD',"DBC"]:
                            Rate = 0
                        else:
                            Rate *= 0.5
                ...
                #print("->", investData['stock_code'], " ==> TargetRate:", TargetRate , " * Rate:", Rate)
                ...
                if IsBuyGo == True  and len(NowInvestList) < len(InvestStockList) and int(date_object.strftime("%Y")) >= StartYear:
                    ...
                    if BackTestType == 'A':
                        ...
                        if IsHell == True:
                            ...
                            if stock_code in ['SPY',"IYK"]:
                                Rate = 0.25
                            elif stock_code in ['TLT',"IEF"]:
                                Rate = 1.75
