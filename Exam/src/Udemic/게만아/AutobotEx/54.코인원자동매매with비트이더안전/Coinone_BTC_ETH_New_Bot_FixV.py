...
#파일 경로입니다.
botdata_file_path = "/var/autobot/Coinone_Safe_Data.json"
try:
...
TotalMoeny = myCoinone.GetTotalMoney(balances) #총 원금
...
##########################################################################################################
    ...
    #if myCoinone.IsHasCoin(balances,coin_ticker) == True: 
    ############################# FixV ####################################
    if BotDataDict[coin_ticker+"_HAS"] == True and myCoinone.IsHasCoin(balances,coin_ticker) == True:
        print("전략으로 매수한 경우!")
        ...
        if IsSellGo == True:
            ...
            balances = myCoinone.SellCoinMarket(coin_ticker,SellAmt)

            if IsDolpaCut == True:
                msg = coin_ticker + " 코인원 안전 전략 봇 : 조건을 하향 돌파 불만족하여 매도처리 했어요!! 현재 수익률 : 약 " + str(round(revenue_data['revenue_rate'],2)) + "% 수익금 : 약" + str(format(round(revenue_data['revenue_money']), ',')) + "원"
                print(msg)
                line_alert.SendMessage(msg)
            else:
                            
                msg = coin_ticker + " 코인원 안전 전략 봇 : 조건을 불만족하여 매도처리 했어요!! 현재 수익률 : 약 " + str(round(revenue_data['revenue_rate'],2)) + "% 수익금 : 약" + str(format(round(revenue_data['revenue_money']), ',')) + "원"
                print(msg)
                line_alert.SendMessage(msg)
        ...
        if IsMaDone == True :
            ...
            ########################################################################################
            #현재 투자 평가금을 구한다!
            NowMoney = myCoinone.GetCoinNowRealMoney(balances,coin_ticker)
    ...
    if coin_data['fix_rate'] > 0:
        ...
        RealMoney = myCoinone.GetCoinNowRealMoney(balances,coin_ticker)
        
        #잔고가 없다면 고정 비중만큼 매수한다!
        if myCoinone.IsHasCoin(balances,coin_ticker) == False: 
            ...
            msg = coin_ticker + " 코인원 안전 전략 봇 :  고정비중이 없어 투자!!"
            print(msg)
            line_alert.SendMessage(msg)
                ...
                if Rate != coin_data['fix_rate']:
                    ...
                    if GapRate < 0:
                        ...
                        if GapMoney >= minmunMoney and abs(GapRate) >= (coin_data['fix_rate'] / 10.0): 
                            ...
                            #그 갭만큼 수량을 구해서 
                            GapAmt = GapMoney / myCoinone.GetCurrentPrice(coin_ticker)

                            #시장가 매도를 한다.
                            balances = myCoinone.SellCoinMarket(coin_ticker,GapAmt)

                            
                            line_alert.SendMessage(coin_ticker + " 코인원 안전 전략 봇 : 리밸런싱 진행!!!! 일부 매도!" )
                    ...
                    #갭이 양수면 비트코인 비중이 적으니 추매할 필요가 있는 경우
                        ...
                        if GapMoney >=  minmunMoney and abs(GapRate) >= (coin_data['fix_rate'] / 10.0):
                            ...
                            balances = myCoinone.BuyCoinMarket(coin_ticker,GapMoney)
                            
                            line_alert.SendMessage(coin_ticker + " 코인원 안전 전략 봇 : 리밸런싱 진행!!! 일부 매수!" )
