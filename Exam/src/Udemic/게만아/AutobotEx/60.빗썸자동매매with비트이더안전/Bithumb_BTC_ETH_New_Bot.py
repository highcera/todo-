...
#파일 경로입니다.
botdata_file_path = "/var/autobot/BithumbSafe_Data.json"
try:
...
##########################################################################################################
    ...
    if myBithumb.IsHasCoin(balances,coin_ticker) == True: 
        ...
        if IsSellGo == True:
            ...
            if IsDolpaCut == True:
                msg = coin_ticker + " 빗썸 안전 전략 봇 : 조건을 하향 돌파 불만족하여 모두 매도처리 했어요!! 현재 수익률 : 약 " + str(round(revenue_data['revenue_rate'],2)) + "% 수익금 : 약" + str(format(round(revenue_data['revenue_money']), ',')) + "원"
                print(msg)
                line_alert.SendMessage(msg)
            else:
                            
                msg = coin_ticker + " 빗썸 안전 전략 봇 : 조건을 불만족하여 모두 매도처리 했어요!! 현재 수익률 : 약 " + str(round(revenue_data['revenue_rate'],2)) + "% 수익금 : 약" + str(format(round(revenue_data['revenue_money']), ',')) + "원"
                print(msg)
                line_alert.SendMessage(msg)
