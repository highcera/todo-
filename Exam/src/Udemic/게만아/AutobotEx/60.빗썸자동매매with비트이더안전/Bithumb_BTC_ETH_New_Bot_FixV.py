...
# UTC 시간을 가져온 후 9시간을 더해 한국 시간으로 변환
utc_time = datetime.utcnow()
korean_time = utc_time + timedelta(hours=9)

day_n = korean_time.day
hour_n = korean_time.hour
min_n = korean_time.minute

print("hour_n:", hour_n)
print("min_n:", min_n)

# YYYYMMDD 형식의 날짜 문자열 생성
day_str = korean_time.strftime("%Y%m%d")
...
#파일 경로입니다.
botdata_file_path = "/var/autobot/Bithumb_Safe_Data.json"
try:
...
TotalMoeny = myBithumb.GetTotalMoney(balances) #총 원금
...
##########################################################################################################
    ...
    df_day = myBithumb.GetOhlcv(coin_ticker,'1d')
    time.sleep(0.1)
    ...
    Ma3_before = myBithumb.GetMA(df_day,3,-3) 
    Ma3 = myBithumb.GetMA(df_day,3,-2)  


    Ma5_before = myBithumb.GetMA(df_day,5,-3) 


    Ma5 = myBithumb.GetMA(df_day,5,-2)   

    Ma6_before = myBithumb.GetMA(df_day,6,-3) 
    Ma6 = myBithumb.GetMA(df_day,6,-2)   

    Ma10_before = myBithumb.GetMA(df_day,10,-3)   
    Ma10 = myBithumb.GetMA(df_day,10,-2)   
    
    Ma19_before = myBithumb.GetMA(df_day,19,-3) 
    Ma19 = myBithumb.GetMA(df_day,19,-2) 
    Ma20 = myBithumb.GetMA(df_day,20,-2) 

    Ma_Last = Ma19
    


    Ma50_before = myBithumb.GetMA(df_day,50,-3) 
    Ma50 = myBithumb.GetMA(df_day,50,-2)

    Ma60 = myBithumb.GetMA(df_day,60,-2)

    
    Ma70 = myBithumb.GetMA(df_day,70,-2)
    
    Rsi_before = myBithumb.GetRSI(df_day,14,-3) 
    Rsi = myBithumb.GetRSI(df_day,14,-2) 
    ...
    #현재가를 구하다
    NowCurrentPrice = myBithumb.GetCurrentPrice(coin_ticker)
    ...
    #if myBithumb.IsHasCoin(balances,coin_ticker) == True: 
    ############################# FixV ####################################
    if BotDataDict[coin_ticker+"_HAS"] == True and myBithumb.IsHasCoin(balances,coin_ticker) == True:
        print("전략으로 매수한 경우!")
        ...
        if IsSellGo == True:
            ...
            AllAmt = myBithumb.GetCoinAmount(balances,coin_ticker)  #현재 수량
            ...
            if IsDolpaCut == True:
                msg = coin_ticker + " 빗썸 안전 전략 봇 : 조건을 하향 돌파 불만족하여 매도처리 했어요!! 현재 수익률 : 약 " + str(round(revenue_data['revenue_rate'],2)) + "% 수익금 : 약" + str(format(round(revenue_data['revenue_money']), ',')) + "원"
                print(msg)
                line_alert.SendMessage(msg)
            else:
                            
                msg = coin_ticker + " 빗썸 안전 전략 봇 : 조건을 불만족하여 매도처리 했어요!! 현재 수익률 : 약 " + str(round(revenue_data['revenue_rate'],2)) + "% 수익금 : 약" + str(format(round(revenue_data['revenue_money']), ',')) + "원"
                print(msg)
                line_alert.SendMessage(msg)
        ...
        if IsMaDone == True :
            ...
            ########################################################################################
            #현재 투자 평가금을 구한다!
            NowMoney = myBithumb.GetCoinNowRealMoney(balances,coin_ticker)
            ...
            if BuyMoney >= minmunMoney:
                ...
                msg = coin_ticker + " 빗썸 안전 전략 봇 :  조건 만족 하여 매수!! " + str(BuyMoney) + "원어치 매수!"
            ...
            if hour_n == 0 and min_n == 0:
                msg = coin_ticker + " 빗썸 안전 전략 봇 :  조건 만족하지 않아 현금 보유 합니다!"
                print(msg)
                line_alert.SendMessage(msg)
    ...
    if coin_data['fix_rate'] > 0:
        ...
        RealMoney = myBithumb.GetCoinNowRealMoney(balances,coin_ticker)
        
        #잔고가 없다면 고정 비중만큼 매수한다!
        if myBithumb.IsHasCoin(balances,coin_ticker) == False: 
            ...
            msg = coin_ticker + " 빗썸 안전 전략 봇 :  고정비중이 없어 투자!!"
            print(msg)
            line_alert.SendMessage(msg)
                ...
                if Rate != coin_data['fix_rate']:
                    ...
                    if GapRate < 0:
                        ...
                        if GapMoney >= minmunMoney and abs(GapRate) >= (coin_data['fix_rate'] / 10.0): 
                            ...
                            #그 갭만큼 수량을 구해서 
                            GapAmt = GapMoney / myBithumb.GetCurrentPrice(coin_ticker)

                            #시장가 매도를 한다.
                            balances = myBithumb.SellCoinMarket(coin_ticker,GapAmt)

                            
                            line_alert.SendMessage(coin_ticker + " 빗썸 안전 전략 봇 : 리밸런싱 진행!!!! 일부 매도!" )
                    ...
                    #갭이 양수면 비트코인 비중이 적으니 추매할 필요가 있는 경우
                        ...
                        if GapMoney >=  minmunMoney and abs(GapRate) >= (coin_data['fix_rate'] / 10.0):
                            ...
                            balances = myBithumb.BuyCoinMarket(coin_ticker,GapMoney)
                            
                            line_alert.SendMessage(coin_ticker + " 빗썸 안전 전략 봇 : 리밸런싱 진행!!! 일부 매수!" )
