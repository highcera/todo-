...
##########################################################################################################
    ...
    df = GetOhlcv2(binanceX,coin_ticker, '1d', 2018, 11, 25, 0, 0)
    print(len(df))
    ...
    for i in range(len(df)):
        ...
        if IsBuy == True:
            ...
            #비트코인의 경우
                ...
                if ((df['high'].iloc[i-2] > df['high'].iloc[i-1] and df['low'].iloc[i-2] > df['low'].iloc[i-1]) or (df['open'].iloc[i-1] > df['close'].iloc[i-1] and df['open'].iloc[i-2] > df['close'].iloc[i-2]) ) :
                    IsSellGo = True
