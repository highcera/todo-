...
BOT_NAME = Common.GetNowDist() + "_MySimplePowerKr"
...
for stock_info in MyPortfolioList:
    ...
    if len(stock_data) == 1:
        ...
        stock_info['stock_target_rate'] *= Rate
...
for stock_info in MyPortfolioList:
    ...
    for stock_code in Top_stocks.index:
        ...
        if  stock_code == stock_info['stock_code']:
            
            stock_info['stock_target_rate'] += ((1.0 - TotalRate) * 0.5) #나머지 현금의 절반의 비중을 더 준다
        
            
    #달러 자산!
    if stock_info['stock_code'] == DollorStock:
