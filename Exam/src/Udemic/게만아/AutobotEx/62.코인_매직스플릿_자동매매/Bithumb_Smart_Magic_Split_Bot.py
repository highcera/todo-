        ...
        if MagicDataInfo['coin_ticker'] == coin_ticker:
            ...
            for MagicData in MagicDataInfo['MagicDataList']:
                if MagicData['Number'] == 1: #1차수를 찾아서!
                    ...
                    if myBithumb.IsHasCoin(balances,coin_ticker) == False: #잔고가 0이라면 2차이후의 차수 매매는 없는거니깐 초기화!
            ...
            for MagicData in MagicDataInfo['MagicDataList']:
                ...
                if MagicData['IsBuy'] == True:
                    ...
                    if CurrentRate >= PickSplitMeta['target_rate'] and myBithumb.IsHasCoin(balances,coin_ticker) == True and (revenue_data['revenue_money'] + MagicDataInfo['RealizedPNL']) > 0 :
