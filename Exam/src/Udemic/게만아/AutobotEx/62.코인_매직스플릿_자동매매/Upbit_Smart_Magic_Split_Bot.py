...
BOT_NAME = "Upbit_SmartMagicSplitBot"
...
for stock_data in TargetStockList:
    ...
    df = pyupbit.get_ohlcv(coin_ticker,interval="day")
    #####################################
    prevClose = df['close'].iloc[-2] #전일 종가
    
    ### 이동평균선구하기 ###
    
    Ma5_Before = myUpbit.GetMA(df,5,-3) #전전일 기준
    Ma5 = myUpbit.GetMA(df,5,-2) #전일 기준
    
    Ma20_Before = myUpbit.GetMA(df,20,-3) #전전일 기준
    Ma20 = myUpbit.GetMA(df,20,-2) #전일 기준
    
    Ma60_Before = myUpbit.GetMA(df,60,-3) #전전일 기준
    Ma60 = myUpbit.GetMA(df,60,-2) #전일 기준
    #####################################
        ...
        if MagicDataInfo['coin_ticker'] == coin_ticker:
            ...
            for MagicData in MagicDataInfo['MagicDataList']:
                if MagicData['Number'] == 1: #1차수를 찾아서!
                    ...
                    if myUpbit.IsHasCoin(balances,coin_ticker) == False: #잔고가 0이라면 2차이후의 차수 매매는 없는거니깐 초기화!
            ...
            for MagicData in MagicDataInfo['MagicDataList']:
                ...
                if MagicData['IsBuy'] == True:
                    ...
                    if CurrentRate >= PickSplitMeta['target_rate'] and myUpbit.IsHasCoin(balances,coin_ticker) == True and (revenue_data['revenue_money'] + MagicDataInfo['RealizedPNL']) > 0 :
