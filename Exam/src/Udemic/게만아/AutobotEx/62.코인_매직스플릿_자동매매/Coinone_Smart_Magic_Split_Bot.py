...
'''
import myCoinone   #우리가 만든 함수들이 들어있는 모듈
import json
import pprint
import line_alert
import time
...
BOT_NAME = "Coinone_SmartMagicSplitBot"
...
for stock_data in TargetStockList:
    ...
    df = myCoinone.GetOhlcv(coin_ticker,'1d')
    #####################################
    prevClose = df['close'].iloc[-2] #전일 종가
    
    ### 이동평균선구하기 ###
    
    Ma5_Before = myCoinone.GetMA(df,5,-3) #전전일 기준
    Ma5 = myCoinone.GetMA(df,5,-2) #전일 기준
    
    Ma20_Before = myCoinone.GetMA(df,20,-3) #전전일 기준
    Ma20 = myCoinone.GetMA(df,20,-2) #전일 기준
    
    Ma60_Before = myCoinone.GetMA(df,60,-3) #전전일 기준
    Ma60 = myCoinone.GetMA(df,60,-2) #전일 기준
    #####################################
        ...
        if MagicDataInfo['coin_ticker'] == coin_ticker:
            ...
            for MagicData in MagicDataInfo['MagicDataList']:
                if MagicData['Number'] == 1: #1차수를 찾아서!
                    ...
                    if MagicData['IsBuy'] == False and MagicDataInfo['Date'] != day_n: #매수하지 않은 상태라면 매수를 진행한다!
                        ...
                        if prevOpen < prevClose and (prevClose >= Ma5 or Ma5_Before <= Ma5):
                            ...
                            if myCoinone.IsHasCoin(balances,coin_ticker) == True:
                                ...
                                MagicData['EntryPrice'] = myCoinone.GetAvgBuyPrice(balances,coin_ticker)
                                MagicData['EntryAmt'] = myCoinone.GetCoinAmount(balances,coin_ticker) 
                                ...
                                MagicData['EntryAmt'] = abs(myCoinone.GetCoinAmount(balances,coin_ticker)  - coin_amt) #진입 수량!
                    ...
                    if myCoinone.IsHasCoin(balances,coin_ticker) == False: #잔고가 0이라면 2차이후의 차수 매매는 없는거니깐 초기화!
            ...
            for MagicData in MagicDataInfo['MagicDataList']:
                ...
                if MagicData['IsBuy'] == True:
                    ...
                    if CurrentRate >= PickSplitMeta['target_rate'] and myCoinone.IsHasCoin(balances,coin_ticker) == True and (revenue_data['revenue_money'] + MagicDataInfo['RealizedPNL']) > 0 :
                        ...
                        #모든 주문 취소하고
                        #myCoinone.CancelCoinOrder(coin_ticker)
                        #time.sleep(0.2)
