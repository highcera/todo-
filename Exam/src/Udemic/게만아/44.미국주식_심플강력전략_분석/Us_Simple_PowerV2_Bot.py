...
for stock_code in StockCodeList:
    ...
    if stock_code == "TQQQ":
        ...
        asset['stock_target_rate'] = 0.3
    elif stock_code == "PFIX":
        asset['stock_target_rate'] = 0.1
    elif stock_code == "TMF":
        asset['stock_target_rate'] = 0.1
...
for stock_info in MyPortfolioList:
    ...
    Ma140_Before = Common.GetMA(df,140,-3)
    Ma140 = Common.GetMA(df,140,-2)
    Ma200_Before = Common.GetMA(df,200,-3)
    Ma200 = Common.GetMA(df,200,-2)
    ...
    #  단! 배당ETF인 SCHD의 경우 이평선에 의한 감산을 하지 않는다!!!  
    if stock_code in ['TQQQ', 'PFIX','TMF']:            
        if Ma200 > prevClose:
            Rate *= 0.5
            CutCnt += 1

        if Ma140_Before > Ma140:
            Rate *= 0.5
            CutCnt += 1     
    
    if CutCnt == 2 and stock_code in ['TQQQ', 'PFIX','TMF']:
        if stock_code == "TQQQ":
            TQQQ_OUT = True     
...
# # 그런데 TQQQ가 감산 2연타를 맞았다면 추가로 PFIX에 35%의 비중을 더해준다!
...
if PFIX_OUT == True and TMF_OUT == False:
    for stock_info in MyPortfolioList:
        ...
        if stock_code == "TMF":
            ...
            if TQQQ_OUT == False:
                stock_info['stock_target_rate'] += 35
...
if Is_Rebalance_Go == True and IsMarketOpen == True:
    ...
    '''
리밸런싱 이후 월 중에 아래 상황 발생시! (월 중 리밸런싱!)
 배당 ETF인 SCHD를 제외한 나머지 자산을 매일 매일 체크한다!
 전전일 종가가 120일선 위에 있었는데 전일 120일선 밑에 종가가 형성된 자산은 보유 수량의 50% 매도!
 or
 전전일 종가가 200일선 위에 있었는데 전일 200일선 밑에 종가가 형성된 자산은 보유 수량의 50% 매도!
 or
 전일 종가 기준 ​140일선이 상승하다가 감소했다면 보유 수량의 50% 매도!
 or
 전일 종가 기준 175일선이 상승하다가 감소했다면 보유 수량의 50% 매도!
 매도할 50%의 보유 수량이 4주 이하라면 모두 매도!!
'''
