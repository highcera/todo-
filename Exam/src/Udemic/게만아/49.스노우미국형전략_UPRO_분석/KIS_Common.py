...
#설정 파일 정보를 읽어 옵니다.
with open('myStockInfo.yaml', encoding='UTF-8') as f:
...
def GetAppKey(dist = "REAL"):
    ...
    return stock_info[dist + "_APP_KEY"]
'''
...
def GetAppSecret(dist = "REAL"):
    ...
    return stock_info[dist + "_APP_SECRET"]
'''
...
def GetPrdtNo(dist = "REAL"):
    ...
    return stock_info[dist + "_ACNT_PRDT_CD"]
'''
...
def GetTokenPath(dist = "REAL"):
    ...
    return stock_info[dist + "_TOKEN_PATH"]
...
#넘겨받은 날짜에서 이전/이후 날짜를 구해서 리턴! (미래의 날짜를 구할 일은 없겠지만..)
def GetFromDateStr(now, type= "NONE" , days=100):
...
def GetOhlcv(area, stock_code, limit = 500, adj_ok = "1"):
    ...
    try:
        ...
        if area == "US":
            ...
            try:
                print("----First try----")
                df = GetOhlcv2(area,stock_code,Adjlimit,adj_ok)#새로 대체!
                ...
                if df is None or len(df) == 0:
                    ...
                    try:
                        print("----Second try----")
                        df = KisUS.GetOhlcvNew(stock_code,"D",limit,adj_ok) 
                    ...
                    if except_riase == True:
                        ...
                        if df is None or len(df) == 0:
                            df = KisUS.GetOhlcv(stock_code,"D",adj_ok)
            ...
            try:
                ...
                if df is None or len(df) == 0:
                    ...
                    if except_riase == True:
                        print("----Third try----")
                        df = KisKR.GetOhlcvNew(stock_code,"D",limit,adj_ok) #새로 대체!
...
def SaveAutoLimitDoAgainData(AutoLimitData):
    ...
    #파일 경로입니다.
    auto_order_file_path = AutoLimitData['Area'] + "_" + AutoLimitData['NowDist'] + "_" + AutoLimitData['BotName'] + "AutoOrderList.json"
    ...
    time.sleep(random.random()*0.1)
    bot_path_file_path = "BotOrderListPath.json"
