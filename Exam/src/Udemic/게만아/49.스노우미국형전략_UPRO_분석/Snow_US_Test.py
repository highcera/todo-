...
TaxAdd = True #양도세 반영 여부!
MoneyForTaxCalc = 0 #양도세 계산을 위한 손익을 저장할 변수
FreeTax = 2100 #환율을 1200원으로 퉁쳐서 $2100 가 면세 기준!


InvestStockList = ["TIP","UPRO","TYD","VNQ","SPTL","VGIT","PFIX","RISR","USHY","VCIT","VWOB","BNDX","BWX","SGOV","DBC"]
...
for stock_code in InvestStockList:
    ...
    #################################################################
    #################################################################
    df = Common.GetOhlcv("US", stock_code,6000) 
    #################################################################
    #################################################################
...
for date in combined_df.index.unique():
    ...
    exclude_stock_codes = ["UPRO","TYD","VNQ","DBC"] #안전자산에 포함되면 안되는 종목들 즉 공격자산들을 넣으면 된다!
